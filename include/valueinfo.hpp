...
namespace nlohmann {
template <>
struct adl_serializer<ValueInfoOption> {
    static void
    to_json(json &j, const ValueInfoOption &i)
    {
        j = json{
            {"name", i.name},
            {"value", i.value},
...
template <>
struct adl_serializer<ValueInfo> {
    static void
    to_json(json &j, const ValueInfo &i)
    {
        j = json::object();
...
template <>
struct adl_serializer<ValueInfoListItem> {
    static void
    to_json(json &j, const ValueInfoListItem &i)
    {
        j         = json::object();
        j["name"] = i.name;
        j["info"] = i.info;
...
struct adl_serializer<Value> {
    static void
    from_json(const json &j, Value &i)
    {
        if (!j.is_object()) {
            return;
        }
...
template <>
struct adl_serializer<ValueSetInfo> {
    static void
    from_json(const json &j, ValueSetInfo &i)
    {
        if (!j.is_object()) {
            return;
        }
